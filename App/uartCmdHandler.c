...
{
	if (nRemReadUART < nBytes) /* if no bytes to get */
	{
		DEBUG_PRINT(" nRemReadUART = %d , nBytes = %d \r\n", nRemReadUART,
				nBytes);
		return false;
	}

	memcpy(parm, rdpUART, nBytes); /* copy from message */
	rdpUART += nBytes; /* bump pointer */
	nRemReadUART -= nBytes; /* bump count */
...
{
	if (nRemWriteUART < nBytes)	// Send the existing packet
	{
		return uartSendResp();
	}

	memcpy(wrpUART, parm, nBytes); /* copy to message */
	wrpUART += nBytes; /* bump pointer */
	nRemWriteUART -= nBytes; /* bump count */
...
static void cmdUARTRead(CMD1_TYPE cmd1)
{
	...
	rdpUART = &pUartMsg->msg.payload.data[sizeof(pUartMsg->msg.payload.cmd)];
	wrpUART = &pUartMsg->msg.payload.data[sizeof(pUartMsg->msg.payload.cmd)];
	nRemReadUART = pUartMsg->msg.head.length - 2; /* number of read bytes */
	/* no. of write bytes */
	nRemWriteUART = (uint16_t) (sizeof(pUartMsg->msg.payload) - sizeof(pUartMsg->msg.payload.cmd));
