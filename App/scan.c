...
static int Scan_SetPGAGain(uint8_t gain_val)
{
	...
	do
	{
		result = ads1255_SetReadContinuous(false);
	...
	if(result == PASS)
	{
		...
		do
		{
			result = ads1255_SetPGAGain(gain_val);
		}while((result != PASS) && (retry_count++ < PGA_SET_RETRY_COUNT));
		...
		if(result == PASS)
		{
			...
			do
			{
				result = ads1255_SetReadContinuous(true);
			}while((result != PASS) && (retry_count++ < PGA_SET_RETRY_COUNT));
		}
	}
...
static int Scan_SetupCalibScan(float* ambt1 , float* dett1 , float* boardt1 , float* humt1)
{
	...
	if(ads1255_Wakeup_NoDelay() != PASS)	// Wake-up ADS1255 takes about 32usec
	{
#ifdef NIRSCAN_INCLUDE_BLE
		bleNotificationHandler_sendErrorIndication(NNO_ERROR_SCAN,
	 ...
	 */
{
	...
	curScanData.black_pattern_period = NUM_BP_PER_FRAME + 1;
	 ...
	 /* If scan button is kept pressed during start-up, apply default config */
	 if(MAP_GPIOPinRead( GPIO_PORTQ_BASE, GPIO_PIN_3) == 0)
	 {
		 ...
		 cfg.scanCfg.wavelength_start_nm = 900;
		 cfg.scanCfg.wavelength_end_nm = 1700;
	 ...
	 */
{
	...
	if(pCfg->scanCfg.scan_type != SLEW_TYPE)
	{
		...
		if((pCfg->scanCfg.wavelength_end_nm > MAX_WAVELENGTH_NM) || (pCfg->scanCfg.wavelength_start_nm < MIN_WAVELENGTH_NM))
			return FAIL;
		...
		scan_total_frames = curScanData.num_patterns/NUM_BP_PER_FRAME + 1;
...
static int Scan_AdjustPGAGain(int32_t max_adc_data)
{
	...
	{
#ifdef NIRSCAN_INCLUDE_BLE
		bleNotificationHandler_sendErrorIndication(NNO_ERROR_ADC, (int16_t)result);
#endif
		nnoStatus_setErrorStatusAndCode(NNO_ERROR_ADC, true, (int16_t)result);
		//curScanData.pga get stored in the .CSV or .DAT scan result file;
		//so make it 0 so that the error is evident to someone looking at it later
		curScanData.pga = 0;
	}
	else
	{
		curScanData.pga = (uint8_t)temp_pga;
	}
